function onl_write_background_2_models(varargin)
    ...
    left_pred_id = onl_newpredictor('left_pred_name',left_pred_model,in_stream,predict_at);
    right_pred_id = onl_newpredictor('right_pred_name',right_pred_model,in_stream,predict_at);
    ...
    start(timer('ExecutionMode','fixedRate', 'Name',[pred_name '_timer'], 'Period',1/update_freq, ...
        'StartDelay',start_delay, 'TimerFcn',@(timer_handle,varargin) write_data(in_stream,out_form,result_writer,left_pred_id,right_pred_id,streamid,timer_handle,verbose,empty_result_value)));
    
    function write_data(stream,fmt,result_writer,left_pred_id, right_pred_id,stream_id,timer_handle,verbose,empty_result_value)
    try
        ...
        p_left = evalin('base', 'left_pred_name');
        if p_left.predictorid ~= left_pred_id
            error('Predictor left changed.'); end
        p_right = evalin('base', 'right_pred_name');
        if p_right.predictorid ~= right_pred_id
            error('Predictor right changed.'); end
    
        % make a prediction
        y1 = onl_predict('left_pred_name',fmt,~verbose,empty_result_value);
        y2 = onl_predict('right_pred_name',fmt,~verbose,empty_result_value);
        % and write it out
        try
            result_writer(y1, y2);
        catch e
