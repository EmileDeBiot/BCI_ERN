function training_model(testedHand, files)
    ...
    if select_all_channel
        ...
        if artifact_correction
            ...
            selected_channels{end+1} = 'EX5';
            selected_channels{end+1} = 'EX6';
            selected_channels{end+1} = 'EX7';
    ...
    for i = 1:length(files)
        dataset = eeg_load_xdf(strcat(data_path,files{i},'.xdf'), 'streamname', 'BioSemi');
        if i == 1
            data = dataset;
        else
    ...
    approach = {'FBCSP', ...
        ...
        'Rereferencing',{'ReferenceChannels',{'EX1','EX2'},'KeepReference',true}, ...
        'FIRFilter','off', ...
        'EpochExtraction',{'TimeWindow',[0.5,4]}, ...
        }, ...
        'Prediction',{ ...
    ...
    % If The data lacks markers, you can add them like this using the EEGLAB function eeg_addnewevents
    % % Adding rest markers to the eeg data
    % latencies = [];
    % for i = 1:length(data.event)
    %     if strcmp(data.event(i).type, 'cross')
    %         latencies = [latencies data.event(i).latency];
    %     end
    % end
    % data = eeg_addnewevents(data, {latencies}, {'rest'});
