function run_robotic_hand(varargin)
...
persistent lib;
global state;
roboticHand=1;  %state of the robotic hand (connected or not) for testing / debugging purpose
...
opts = arg_define(varargin, ...
    ...
    arg({'tested_hand', 'Hand'}, 'right', [], 'The tested hand'), ...
    ...
    arg({'hold_hand'},0,[],'If hold == 1, the robotic hand stays in its state until the state changes. Otherwhise it just activates and go back to rest.'),...
    arg({'finger'},2,[],'Finger ID'), ...
    arg({'thresholds'},[0.3 0.5 0.7],[],'Thresholds for the activation of the hand'));
...
robotic_hand = instrfind;
if isempty(robotic_hand)
    if(roboticHand)
        robotic_hand = ardui('COM12');
    end
% else
%     delete(instrfind);
%     robotic_hand = ardui('COM9');
 end
disp('taking my time'); pause(0.5); 

% start background writer job

onl_write_background( ...
    'ResultWriter',@(y)send_samples(outlet, robotic_hand, opts.tested_hand, opts.thresholds, y),...
...
function send_samples(outlet,robotic_hand, tested_hand,thresholds,y)
    if ~isempty(y)
        outlet.push_chunk(y');
        if strcmp(tested_hand, 'both')
            if (y-1) <= thresholds(1)
                comm(robotic_hand,'l2');
            elseif (y-1) >= thresholds(3) 
                comm(robotic_hand,'r2');
            end
        elseif (y-1) >= thresholds(2)
            if strcmp(tested_hand, 'left')
                comm(robotic_hand,'l2');
            elseif strcmp(tested_hand, 'right')
                comm(robotic_hand,'r2');
            end
        end
    end

% function send_samples2(outlet, robotic_hand, hold_hand, finger, thresholds, y1, y2)
%     roboticHand = 1;
%     if ~isempty(y1) && ~isempty(y2)
%         outlet.push_chunk(y1');
%         outlet.push_chunk(y2');
%         if ((y1 - 1) >= thresholds(2)) && ((y2 - 1) < thresholds(2))    % gauche actif et droit inactif => on active la gauche
%             if(roboticHand)
%                 disp('l2');
%                 comm(robotic_hand, 'l2');
%             end
%         elseif ((y2 - 1) >= thresholds(2)) && ((y1 - 1) < thresholds(2))
%             if(roboticHand)
%                 disp('r2');
%                 comm(robotic_hand, 'r5');
%             end
%             
%         else
%             disp('Rien');
