function [outlet, opts] = init_outlet(varargin)
    ...
    opts = arg_define(varargin, ...
        arg({'left_pred_model','LeftModel'}, 'lastmodel', [], 'Predictive model. As obtained via bci_train or the Model Calibration dialog.','type','expression'), ...
        arg({'right_pred_model','RightModel'}, 'lastmodel', [], 'Predictive model. As obtained via bci_train or the Model Calibration dialog.','type','expression'), ...
    ...
    % load the models
    left_model = utl_loadmodel(opts.left_pred_model);
    right_model = utl_loadmodel(opts.right_pred_model);
    
    % check if channel labels make sense for the models
    if strcmp(opts.out_form,'distribution')
        if isfield(left_model,'classes') && ~isempty(left_model.classes)
            if length(opts.channel_names) ~= length(left_model.classes)
                disp('The number of classes provided by the left model does not match the number of provided channel names; falling back to default names.');
                opts.channel_names = cellfun(@(k)['class' num2str(k)],num2cell(1:length(left_model.classes),1),'UniformOutput',false);
            end
        end
        if isfield(right_model,'classes') && ~isempty(right_model.classes)
            if length(opts.channel_names) ~= length(right_model.classes)
                disp('The number of classes provided by the right model does not match the number of provided channel names; falling back to default names.');
                opts.channel_names = cellfun(@(k)['class' num2str(k)],num2cell(1:length(right_model.classes),1),'UniformOutput',false);
            end
        end
    else
        if isfield(left_model,'classes') && ~isempty(left_model.classes)
            if length(opts.channel_names) ~= 1
                disp('A classification left model will produce just one channel if the output is not in distribution form, but a different number of channels was given. Falling back to the default channel label.');
                opts.channel_names = {'class'};
            end        
        end
        if isfield(right_model,'classes') && ~isempty(right_model.classes)
            if length(opts.channel_names) ~= 1
                disp('A classification right model will produce just one channel if the output is not in distribution form, but a different number of channels was given. Falling back to the default channel label.');
                opts.channel_names = {'class'};
            end        
        end
    end
    ...
    try
        if strcmp(opts.source_id,'input_data')
            uid = hlp_cryptohash({left_model.source_data,opts.predict_at,opts.in_stream,opts.out_stream});
        else
